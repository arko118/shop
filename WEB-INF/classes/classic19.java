...
public class classic19 extends HttpServlet{
Connection con=null;
...
ResultSet rs=stmt.executeQuery("select * from category3 where id2="+m.getPid()+" ");
...
while(rs.next()){
	...
	String id11=new Integer(rs.getInt("id2")).toString();
	int id2=new Integer(id11).intValue();

	String id_1=new Integer(rs.getInt("id")).toString();
	...
	case 1901:
	out.println("<html><body ><table  scrolling=No ><tr><td bgcolor=lightgray>   <table  scrolling=No ><tr><td bgcolor=white><table><tr><td align=left bgcolor=grayellow><b>Name:<font color=blue>"+name+"</font> </b></td><th></th><th></th><td align=left bgcolor=grayellow><b>Quantity:<font color=red>"+quantity+"</font></b></td><td></td><td align=right bgcolor=grayellow><b>Price:<font color=red>"+price+"</b></font></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td align=left bgcolor=red><form action=/shopping/servlet/category2add method=get><input type=hidden name=id size=5 value="+id3+"><input type=hidden name=back size=5 value="+pd+"><input type=submit value=Order><input type=text name=qunt size=3 value=0></form></td> </tr></table>");
